// 📁 index.js
import { openModal, closeModal, openModalTwo, closeModalTwo } from "./modal.js";
...
$(".btn-open-modal_2").addEventListener("click", () => {
    console.log("modal2")
    openModalTwo();
});

$(".modal-container_2").addEventListener("click", (event) => {
    if (event.target === $(".modal-container_2")) {
        closeModalTwo();
    }
});

$(".modal-close_2").addEventListener("click", () => {
    closeModalTwo();
});

// 오늘 보지 않기 버튼을 누르면 만료기간이 1일인 쿠키를 생성하고 모달을 닫는다.
$(".modal-stop-button_2").addEventListener("click", () => {
    // 하루 기한의 쿠키를 생성한다. (쿠키 생성시 이름은 modal-closed, 값은 true로 설정한다.)
    // 모달을 닫는다.
    closeModalTwo();
    setCookie("modalClose", "true", 1);
